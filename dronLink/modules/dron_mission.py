...
def _executeMission (self, callback=None, params = None, wait_until_finish = True):
    ...
    if wait_until_finish:
        # Damos un tiempo para que el dron esté en el aire y esperamos
        # a que termine para volver a estado conectado.
        time.sleep(10)
...
def executeMission(self, blocking=True, callback=None, params = None, wait_until_finish = True):
    if blocking:
        self._executeMission(callback, params, wait_until_finish)
    else:
        missionThread = threading.Thread(target=self._executeMission, args=[callback, params, wait_until_finish])
        missionThread.start()
